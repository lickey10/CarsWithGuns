...
public class clsshowcasehelper : MonoBehaviour {
	...
	public void metemit() {
        ...
        ParticleSystem varemitter = null;
		if (varself) {
			varemitter = GetComponentInChildren<ParticleSystem>();
		}
		else {
			if (vargamactor == null) {
			...
			varemitter = vargamactor.GetComponentInChildren<ParticleSystem>();
		}
		if (varemitter == null) {
			Debug.LogError("No emitter", gameObject);
			return;
		}
        varemitter.Play();//.emit = true;
	}
