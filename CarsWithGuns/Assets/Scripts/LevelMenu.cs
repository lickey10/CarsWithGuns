...
public class LevelMenu : MonoBehaviour {
	...
	void Start () {
		...
		//set background
        if(Backgrounds.Length >= gamestate.Instance.getActiveLevel())
	...
	void OnGUI()
	{
		...
		//GUI.Box(new Rect( 3, 3, 450, 40 ), "Cars With Guns" ,customGuiStyle);
		
		//customGuiStyle.normal.textColor = Color.green;
		//GUI.Box(new Rect( 0, 0, 450, 40 ), "Cars With Guns" ,customGuiStyle);
		...
		for(int x = 0; x < numRows; x++)
		{
            ...
            if (gamestate.Instance.GetNumberOfLevels() == 1)//start the only level
            {
			...
			for (int y = 0; (x*MaxColumns) + y < gamestate.Instance.GetNumberOfLevels(); y++)
			{
				...
				else //unlocked
				{
					...
					if (GUI.Button(new Rect(y*buttonWidth+buttonPadding,x*buttonHeight+buttonPadding,buttonWidth-5,buttonHeight-5),((x*MaxColumns)+(y+1)).ToString(),customGuiStyle))
					{
                        ...
                        if (Scenes != null)
                            SceneManager.LoadScene(Scenes[selectedLevel-1].name);
                        else
                            SceneManager.LoadScene("level_"+ selectedLevel.ToString());
